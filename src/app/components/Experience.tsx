...
export default function Experience() {
  ...
  const experiences: Record<ExperienceKey, { name: string; title: string; date: string; url: string; details: string[] }> = {
    ...
    Iker: {
      ...
      details: [
        'Contributed to the design and development of the "Booking-Container" platform, creating a responsive website using Laravel, PHP, Bootstrap, Tailwind, HTML, CSS, and JavaScript.',
    ...
    <main className="mb-90 flex max-sm:ml-[-10px] lg:ml-[-50px] max-sm:mt-[80px] flex-row max-sm:flex-wrap sm:ml-5 max-sm:flex-row md:flex-row items-start ml-[-15px] md:p-12 lg:gap-12 sm:gap-4 md:ml-[-40px] md:gap-1">
      <div className="md:w-3/3 sm:w-1/2 sm:mx-8 max-sm:w-full">
        ...
        <div className="sm:w-full max-w-full  flex flex-col md:flex-row max-sm:flex-col sm:flex-row mt-10 sm:justify-center  lg:items-start">
              ...
              {Object.keys(experiences).map((key) => (
                <li
                  ...
                  className={`border-l-3 pl-3 cursor-pointer transition-all duration-300 ease-in-out max-sm:inline transform ${
                    activeTab === key
                      ? 'border-[#64ffda] text-[#64ffda] scale-105'
                      : 'border-transparent'
                  }`}
          ...
          <div className="sm:w-full ml-7 lg:text-[16px] mt-4 md:mt-0 md:pl-8 sm:text-sm sm:ml-28 md:ml-0 lg:ml-0 sm:mr-[-200px] lg:mr-[80px] md:mr-[-30px]">
            <div>
