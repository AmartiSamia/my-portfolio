...
export default function Skills() {
  ...
  return (
    <main className="mb-90 w-full text-center flex justify-center max-sm:ml-[-10px] max-sm:mt-[80px] flex-row max-sm:flex-wrap sm:ml-5 max-sm:flex-row md:flex-row items-start md:p-12 lg:mx-0 sm:gap-4 md:ml-[-40px] md:gap-1">
      <div className="md:w-full sm:w-full sm:mx-8 max-sm:w-full">
        <h1 className="text-[#64ffda] text-xl font-bold mb-4 text-center items-center flex justify-center">
          ...
          <span id="Name" className="text-[#ccd6f6] text-4xl mb-8">
            My Tech Stack
          </span>
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {Object.keys(techStack).map((category) => (
            (techStack[category as keyof typeof techStack] as TechCategory[]).map((tech, index) => (
              <div
                key={index}
                className="text-center bg-[#112240] p-5 transition-transform transform hover:translate-y-[-8px] hover:scale-105 hover:shadow-[0px_2px_2px_rgba(100,255,218,0.5)] rounded-lg flex flex-col items-center justify-center sm:w-1/6 md:w-1/6 lg:w-1/6"
              >
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {Object.keys(Languages).map((language, index) => (
            <div
              ...
              className="text-center bg-[#112240] p-5 transition-transform transform hover:translate-y-[-8px] hover:scale-105 hover:shadow-[0px_2px_2px_rgba(100,255,218,0.5)] rounded-lg flex flex-col items-center justify-center sm:w-1/6 md:w-1/6 lg:w-1/6"
            >
              <p className="text-white mt-2">{Languages[language]}</p>
            </div>
          ))}
        </div>
      </div>
